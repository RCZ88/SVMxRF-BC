...
def buildTree(data, finalColIndex:int, branchCount = 1, tree:Tree = None):
    ...
    if isPure(afterSort.left, finalColName):
        ...
        print(tree.toString())
        tree.left = buildTree(tree.left.preSortData, finalColIndex, branchCount + 1, tree.left)
    ...
    if isPure(afterSort.right, finalColName):
        ...
        print(tree.toString())
        tree.right = buildTree(tree.right.preSortData, finalColIndex, branchCount + 1, tree.right)
...
def createForest(numberOfBootstraps):
    ...
    endTime = time.time()
    randomForest = RandomForest(treeBootstraps)
