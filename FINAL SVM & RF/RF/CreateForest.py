...
def findBestSort(data, finalCol:str):
    print("Parent Data: ")
    print("Gini Index: ", getGiniIndex(data, finalCol, True), "\n----------------\n")
    print(data)
    dataGathered = []

    for i in range(len(data.columns)-1): #column
        for j in range(len(data)): #row
            ...
            dataGathered.append(Filter(data.columns[i], sortVal, ig, left, right, finalLeaveOfData(left, finalCol), finalLeaveOfData(right, finalCol)))
...
def buildTree(data, finalColIndex:int, branchCount = 1, tree:Tree = None):
    ...
    if isPure(afterSort.left, finalColName):
        ...
        print(tree.toString())
        tree.left = buildTree(tree.left.dataPostFilter, finalColIndex, branchCount + 1, tree.left)
    ...
    if isPure(afterSort.right, finalColName):
        ...
        print(tree.toString())
        tree.right = buildTree(tree.right.dataPostFilter, finalColIndex, branchCount + 1, tree.right)
